...
function Hider() {
    ...
    const mobileMenuClasses = `
        md:hidden bg-indigo-700 w-full px-6 pb-4
    ...
    return (
        <nav aria-label="Main Navigation" className="relative">
            {/* Fixed Navbar */}
            <div className="flex justify-between items-center fixed top-0 left-0 w-full h-[72px] shadow-md px-6 md:px-12 z-50 bg-white/90 dark:bg-blue-950/90 backdrop-blur">
                {/* Logo */}
                            ...
                            <li key={item.name}>
                                <a
                                    ...
                                    className={`px-4 py-2 rounded-xl transition-all duration-300 ease-in-out ${location.pathname === item.link
                                        ...
                                        : "text-gray-400 hover:text-[#854FEE] hover:bg-[#854fee1a]"
                                        }`}
                                >
                                    {item.name}
                                </a>
                ...
                {/* Mode Toggle + Button + Mobile Menu Button */}
                <div className="flex items-center gap-3">
                    {/* Desktop Contact Button */}
                    <a
                        href="#contact"
                        className="hidden md:inline-block px-4 py-2 rounded-lg font-semibold bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 text-white shadow-lg hover:scale-105 transition-transform"
                    >
                        Contact Me
                    </a>
                    ...
                    {/* Mobile Menu Button */}
                    <button
                        className="md:hidden text-gray-700 dark:text-white p-2 z-50 ml-2"
                ...
                <div className="flex flex-col gap-2  font-bold text-gray-200 mt-4">
                    ...
                    {navLinks.map((link) => (
                        <Link
                            ...
                            className={`py-2 pl-2 rounded transition duration-150 hover:bg-indigo-800 hover:text-white ${location.pathname === link.link ? "bg-indigo-800 text-white" : ""
                                }`}
                        >
                            {link.name}
                        </Link>
                    ...
                    <a
                        href="#contact"
                        onClick={() => setOpen(false)}
                        className="py-2 px-2 rounded-lg text-center font-semibold bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 text-white shadow-lg hover:scale-105 transition-transform"
                    >
                        Contact Me
                    </a>
                </div>
